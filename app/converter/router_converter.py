...
def converter():
    if request.method == 'POST':
        ...
        form_from_currency = request.form['from_currency']
        form_to_currency = request.form['to_c']
        form_amount = request.form['amount']
        url = f'https://api.getgeoapi.com/v2/currency/convert?api_key={api_key}&from={form_from_currency}&to={form_to_currency}&amount={form_amount}&format=json'
        ...
        response_to_currency_name = response_json['rates'][form_to_currency]['currency_name']
        response_result_convert = response_json['rates'][form_to_currency]['rate_for_amount']
        response_result_convert_round = round(float(response_result_convert), 2)


        return render_template('converter/converter_page.html', title='Конвертер валюты', result=response_result_convert_round, amount=response_amount_round, from_c_code=response_from_currency_code, from_c_name=response_from_currency_name, to_c_code=form_to_currency, to_c_name=response_to_currency_name, time=last_update_date)
