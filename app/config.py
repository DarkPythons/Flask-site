"""
Модуль, в котором происходит получение данных из окружения env, файла .env
BaseConfigClassInFile: родительский класс, в котором происходить основная настройка окружения
BaseSettingsDataBase: дочерний класс, который берет данные из окружения для базы данных Postgre
BaseSettingsApp: дочерний класс, который берет данные из окружения для основного приложения
SettingByAPICurrency: дочерний класс, который берет данные из окружения для работы с API конвертора
"""
...
# Для лучшей работы requests метода get, укажем заголовки
headers = {
...
class SettingByAPICurrency(BaseConfigClassInFile):
    """Класс для взятия данных из окружения, для настройки API конвертации валюты"""
